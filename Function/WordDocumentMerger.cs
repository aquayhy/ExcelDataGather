...
namespace ExcelDataGather
{
    ...
    public class WordDocumentMerger
    {
        ...
        /// 需要合并的文件
        /// 合并后的输出文件
        public void CopyMerge(string tempDoc, IList<String> arrCopies, string outDoc)
        {
        ...
        /// 需要合并的文件
        /// 合并后的输出文件
        public void InsertMerge(string tempDoc, IList<String> arrCopies, string outDoc)
        {
            object objMissing = Missing.Value;
            object objFalse = false;
        ...
        public void InsertMerge(IList<String> arrCopies, string outDoc,bool outputPDF)
        {
            int lastIndex = arrCopies.Count - 1;
            string tempDoc = arrCopies[lastIndex];
            arrCopies.RemoveAt(lastIndex);
            object objMissing = Missing.Value;
            object objFalse = false;
            object confirmConversion = false;
            object link = false;
            ...
            try
            {
                ...
                SaveAs(outDoc);
                if(outputPDF)
                    objDocLast.ExportAsFixedFormat(outDoc+".pdf", WdExportFormat.wdExportFormatPDF);
                foreach (Document objDocument in objApp.Documents)
                {
                    objDocument.Close(
                      ref objFalse,     //SaveChanges
